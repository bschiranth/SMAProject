...
/* Public Interface - 
* -------------------------
 *  Receiver()  - constructor sets listening endpoint
 * CreateListener() -creates listener but does not start it
 * StartService() -Create CommService and listener and start it
 * defaultServiceAction() - if no other action is done
 * doService() - run the service action
 * getMessage() - gets sender message
 * shutDown() - shuts the receiver down
 * 
 * Maintenance History:
 * --------------------
 * ver 2.2 : Nov 17 2015
 ...
 */
  /** Required Files - 
 * ------------------------------
 * ICommService.cs, Utilities.cs,CommService.cs
 */
using System;
