 ...
 */
/* Public Interface:
*  showMetadata() - puts metadata to string
*  showElement() - displays dbelement by calling showmetadata method
*  show() - enumerable db elements are shown
/*
* Maintenance:
* ------------
* Required Files: 
...
namespace Project4Starter
{
    ...
    public static class DBElementExtensions
    {
        ...
        public static string showMetaData<Key, Data>(this DBElement<Key, Data> elem)
        {
            ...
            if (elem.children.Count() > 0)              //This runs only if the elem has children
            {
                accum.Append(String.Format("\n  Children: "));
                bool first = true;
        ...
        {
            StringBuilder accum = new StringBuilder();
            accum.Append(elem.showMetaData());     //showMetaData is extended to showElement
          ...
          where Data : IEnumerable<T>  // constraint clause
        {
            ...
            if (elem.payload != null)           // The payload needs to have some data for this condition
            {
    ...
    public static class DBEngineExtensions
    {
        ...
        public static void show<Key, Value, Data>(this DBEngine<Key, Value> db)
        {
            foreach (Key key in db.Keys())
            {
                ...
                DBElement<Key, Data> elem = value as DBElement<Key, Data>;  //send value as dbelement to elem
                Write("\n\n  -- key = {0} --", key);
...
#if (TEST_DBEXTENSIONS)                 // Test stub for the db extensions
    ...
    class TestDBExtensions
    {
        static void Main(string[] args)
        {
            ...
            Write(elem1.showElement<int, string>());        //first dbelement is displayed
