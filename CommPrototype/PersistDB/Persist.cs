...
namespace Project4Starter
{
    ...
    namespace Project4Starter
    {
        public class PersistEngine
        {
            ...
            public void loadDB<Key, Value, Data>(DBEngine<Key, Value> db, string file_name){try{
                    ...
                    XElement root = xml.Element("NoSqlDb");
                     if (String.Compare(root.Element("KeyType").Value, typeof(Key).ToString()) == 0)    {
                        if (String.Compare(root.Element("PayloadType").Value, typeof(Data).ToString()) == 0)                        {
                            ...
                            try   { XElement xml_dbElement = kv_pair.Element("Element");
                ...
                WriteLine("\n  An error occured while reading the XML File. Skipping to parse the next DB Element."); }
                            }}else WriteLine("The payload type of database is not same as the payload type of elements in XML file.");}
                    else WriteLine("The Key-type of database is different from the Key-type present in XML file. Cannot Load database.");}
                catch (System.IO.FileNotFoundException){WriteLine("\n  XML File not found. Could not load it to database.");}
